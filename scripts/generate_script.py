...
- Subtitles synced to actual audio duration
"""
...
def build_subtitle_schedule(script, audio_duration):
    all_lines = [script["hook"]] + script["lines"]
    n = len(all_lines)
    spl = audio_duration / n
    schedule = []
...
def generate_video(script_path="script.json", audio_path="narration.mp3", output_path="short.mp4"):
    ...
    if os.path.exists(audio_path):
        ...
        audio_duration = 60.0
        print("[WARN] No audio found, defaulting to 60s")

    subtitle_schedule = build_subtitle_schedule(script, audio_duration)
